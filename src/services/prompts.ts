...
const REPORT_DATA_SCHEMA = `interface ReportData {
  patterns: Pattern[];                      // Pattern = ${PATTERN_VOCAB}
  approachSummary: string;                  // one line: the approach the report describes
  optimalComplexity: { time: string; space: string };
  solvedOptimally: boolean;                 // did the solution reach the optimal approach?
}`;

const REPORT_DATA_EXAMPLE = `{
  "patterns": ["Stack", "Sorting"],
  "approachSummary": "Sort by position, then use a monotonic stack of arrival times.",
