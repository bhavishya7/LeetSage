/**
 * Chat Mode Component (Optional)
 *
 * A secondary interaction mode for free-form questions.
 * Visually distinct from action-button-generated content.
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      for await (const chunk of streamLLMRequest({
        problemContext,
        actionType: 'EXPLAIN_CONCEPT', // Use a general action type for chat
        systemPrompt: '',
      ...
      setMessages(prev =>
        prev.map(m => m.id === assistantId ? { ...m, content: filteredContent } : m)
      );
    } catch (err) {
      setMessages(prev =>
        prev.map(m => m.id === assistantId
          ? { ...m, content: '⚠️ Error generating response. Please try again.' }
          : m
        )
      );
    } finally {
    ...
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-2 space-y-2">
        {messages.length === 0 && (
          <p className="text-xs text-gray-400 text-center mt-4">
            Ask any question about this problem. I won't give you the answer, but I'll help you understand it.
          </p>
        )}
        {messages.map(msg => (
      ...
      <div className="px-3 py-2 border-t flex gap-2">
        ...
        <button
          ...
          className="text-xs px-3 py-1.5 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-40 transition-colors"
        >
          Send
        </button>
      </div>
