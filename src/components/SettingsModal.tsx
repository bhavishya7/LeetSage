 ...
 * Settings Modal
 *
 * Allows users to configure their LLM API key and preferences.
 * The API key is stored in Chrome Storage (local, not synced).
 */
...
const SettingsModal: React.FC<SettingsModalProps> = ({ currentSettings, onSave, onClose }) => {
  ...
  const [provider, setProvider] = useState<'openai' | 'anthropic'>(
    currentSettings?.apiConfig.provider === 'anthropic' ? 'anthropic' : 'openai'
  );
  const [enableStuckTimer, setEnableStuckTimer] = useState(
    currentSettings?.enableStuckTimer ?? true
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const validateApiKey = (key: string, prov: string): string => {
    if (!key.trim()) return 'API key is required';
    if (prov === 'openai' && !key.startsWith('sk-')) return 'OpenAI keys start with "sk-"';
    if (prov === 'anthropic' && !key.startsWith('sk-ant-')) return 'Anthropic keys start with "sk-ant-"';
    return '';
  };
  ...
  const handleSave = async () => {
    ...
    const newSettings: UserSettings = {
      apiConfig: { provider, apiKey: apiKey.trim() },
      enableStuckTimer,
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-sm">
        ...
        <div className="px-4 py-4 space-y-4">
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-medium text-gray-700">Proactive suggestions</p>
              <p className="text-[11px] text-gray-400">Suggest help after 5 min of inactivity</p>
            </div>
            <button
              onClick={() => setEnableStuckTimer(v => !v)}
              className={`w-10 h-5 rounded-full transition-colors ${enableStuckTimer ? 'bg-blue-500' : 'bg-gray-300'}`}
              role="switch"
              aria-checked={enableStuckTimer}
            >
        ...
        <div className="flex gap-2 px-4 py-3 border-t">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 text-xs py-1.5 bg-blue-500 text-white rounded hover:bg-blue-600 disabled:opacity-50 transition-colors"
          >
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
