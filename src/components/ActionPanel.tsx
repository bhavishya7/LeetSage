 ...
 *
 * The primary UI — displays the problem info and action buttons.
 * Action buttons are the main way users interact with LeetSage.
 *
 * Design decisions:
 * - Buttons are prominent and immediately accessible (no typing required)
 * - Used actions show a checkmark so users know what they've tried
 * - Disabled state when no API key or loading
 */
...
const ActionPanel: React.FC<ActionPanelProps> = ({
  ...
  const handleClick = (action: ActionConfig) => {
    if (action.type === 'CHECK_APPROACH') {
      setShowApproachInput(v => !v);
      return;
    }
    ...
    <div className="shrink-0 bg-white border-b border-gray-200 px-3 py-3">
      ...
      {problemContext ? (
        <div className="mb-3">
          <div className="flex items-start justify-between gap-2">
            ...
            <span className={`shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${DIFFICULTY_COLORS[problemContext.difficulty] ?? 'bg-gray-100 text-gray-600'}`}>
      ...
      {!apiKeyConfigured && (
        <button
          ...
          className="w-full mb-3 text-xs text-center py-1.5 bg-orange-50 border border-orange-200 rounded text-orange-700 hover:bg-orange-100 transition-colors"
        >
      ...
      {hintLevel > 0 && (
        <div className="mb-2 flex items-center gap-1 text-xs text-gray-500">
          ...
          {[1, 2, 3].map(l => (
            <span key={l} className={`w-4 h-4 rounded-full flex items-center justify-center text-[10px] ${l <= hintLevel ? 'bg-yellow-400 text-white' : 'bg-gray-200 text-gray-400'}`}>
              {l}
            </span>
          ))}
      ...
      <div className="grid grid-cols-2 gap-1.5">
        {ACTIONS.map(action => {
          ...
          const isDisabled = isLoading || !apiKeyConfigured || !problemContext ||
            (action.type === 'GET_HINT' && hintsExhausted);
          ...
          return (
            <button
              ...
              className={`
                relative flex items-center gap-1.5 px-2 py-2 rounded border text-left transition-all
                ${action.color}
                ${isDisabled ? 'opacity-40 cursor-not-allowed' : 'cursor-pointer'}
                ${isLoading && !isDisabled ? 'animate-pulse' : ''}
              `}
            >
              <span className="text-base leading-none">{action.icon}</span>
              <span className="text-xs font-medium text-gray-700 leading-tight">{action.label}</span>
              {isUsed && (
                <span className="absolute top-1 right-1 text-[10px] text-green-500" aria-label="Used">✓</span>
              )}
            </button>
          );
        })}
      ...
      {/* Check Approach input */}
      {showApproachInput && (
        <div className="mt-2">
          <textarea
            value={approachInput}
            onChange={e => setApproachInput(e.target.value)}
            placeholder="Describe your approach... (e.g., 'I'm thinking of using a hash map to store...')"
            className="w-full text-xs border border-gray-300 rounded p-2 resize-none focus:outline-none focus:ring-1 focus:ring-teal-400"
            rows={3}
          />
          <div className="flex gap-2 mt-1">
            <button
              onClick={handleApproachSubmit}
              disabled={!approachInput.trim() || isLoading}
              className="flex-1 text-xs py-1 bg-teal-500 text-white rounded hover:bg-teal-600 disabled:opacity-40 transition-colors"
            >
              Submit Approach
            </button>
            <button
