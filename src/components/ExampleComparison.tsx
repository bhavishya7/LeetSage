...
interface ExampleComparisonProps { originalExamples: Example[]; }
      ...
      <div className="space-y-1">
        {originalExamples.map((ex, i) => (
          <div key={i} className="bg-gray-50 border border-gray-200 rounded p-2 text-xs font-mono">
            <span className="text-gray-500">Input: </span><span className="text-gray-800">{ex.input}</span><br />
            <span className="text-gray-500">Output: </span><span className="text-gray-800">{ex.output}</span>
            {ex.explanation && <><br /><span className="text-gray-500">Explanation: </span><span className="text-gray-600 font-sans">{ex.explanation}</span></>}
          </div>
        ))}
      </div>
    </div>
